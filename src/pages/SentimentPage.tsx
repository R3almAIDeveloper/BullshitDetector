  ...
  const analyzeSentiment = async () => {
    ...
    if (!apiKey) {
      setResult({
        ...
        negative: 0,
        totalPosts: 0,
        explanation: 'Please add your xAI API key in Settings to run real sentiment analysis.',
      });
      setLoading(false);
      return;
    }
      ...
      const response = await fetch('https://api.x.ai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are a real-time public sentiment analyzer. 
              Analyze current public opinion on the topic: "${topic}".
              Respond **only** with valid JSON in this exact format:
              {
                ...
                "explanation": "1-sentence summary of sentiment trend"
              }
  ...
  return (
    <div className="container mx-auto p-6 max-w-5xl">
      ...
      <p className="text-gray-600 dark:text-gray-400 mb-6">
        Analyze real-time public sentiment on any topic using Grok.
      </p>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border mb-8">
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            placeholder="e.g., Climate Change, Universal Healthcare, AI Regulation..."
            className="flex-1 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:border-gray-600"
            disabled={loading}
          />
          <button
            onClick={analyzeSentiment}
            disabled={loading || !topic.trim()}
            className="px-6 py-3 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2"
          >
            {loading ? (
              <>
      ...
      {result && total > 0 && (
        <>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg border">
            <h3 className="font-semibold mb-2">Topic: {result.topic}</h3>
      ...
      {result && result.explanation?.includes('Please add') && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-lg">
          <p className="text-sm text-yellow-800 dark:text-yellow-300">
            {result.explanation} <a href="/settings" className="underline font-medium">Go to Settings →</a>
          </p>
        </div>
      )}
